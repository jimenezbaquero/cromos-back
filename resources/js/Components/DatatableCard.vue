  ...
  <div class="bg-white rounded-lg overflow-hidden">
    ...
    <div class="hidden md:block overflow-x-auto">
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tr  class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer select-none">
          <th>
            {{$t('preview')}}
          </th>
          <th
        ...
        <tr v-for="item in pagination.data" :key="item.id" class="hover:bg-gray-200">
          <td>
            <img :src="item[image]" :alt="$t('card')+' '+item.number" class="h-24 w-24 object-cover" />
          </td>
          <td
            v-for="column in columns"
            :key="column.key"
